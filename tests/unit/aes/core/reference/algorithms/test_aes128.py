...
from purepython_aes import ReferenceAes128
from purepython_aes.const import AES_128_KEY_SIZE, AES_128_ROUND_COUNT
from tests.unit.aes.core.reference.algorithms.strategies import reference_aes128
...
class TestAes128:
    ...
    def test_aes_128_key_size() -> None:
        assert ReferenceAes128(bytes(AES_128_KEY_SIZE)).__key_size__ == AES_128_KEY_SIZE

    @staticmethod
    def test_aes_128_round_count() -> None:
        assert (
            ReferenceAes128(bytes(AES_128_KEY_SIZE)).__round_count__
            == AES_128_ROUND_COUNT
        )
    ...
    def test_fips_197_aes_128_known_answer(example_plaintext: bytes) -> None:
        ...
        assert ReferenceAes128(key).encrypt_block(example_plaintext) == ciphertext

    @staticmethod
    def test_spa_800_38a_plaintexts(
        spa_800_38a_plaintexts: tuple[bytes, bytes, bytes, bytes],
    ) -> None:
        aes: ReferenceAes128 = ReferenceAes128(
    ...
    @staticmethod
    @given(aes=reference_aes128, block=aes_blocks)
